...
export function validateAccessibility(spec: DiagramSpec, rendered: SvgResult): ValidationResult {
  ...
  if (rendered.alternative.length === 0) {
    issues.push({
      level: 'L4',
      code: 'ACCESSIBILITY_ERROR',
      message: 'alternative list is empty; it must cover every node and edge',
    });
  }
