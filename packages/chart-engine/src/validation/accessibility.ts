...
export function validateAccessibility(spec: ChartSpec, rendered: SvgResult): ValidationResult {
  ...
  if (rendered.tabular.length === 0) {
    issues.push({
      level: 'L4',
      code: 'ACCESSIBILITY_ERROR',
      message: 'tabular alternative is empty; it must read every data row',
    });
  }
