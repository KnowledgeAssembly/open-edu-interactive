...
export function validateAccessibility(spec: TimelineSpec, rendered: SvgResult): ValidationResult {
  ...
  if (rendered.linear.length === 0) {
    issues.push({
      level: 'L4',
      code: 'ACCESSIBILITY_ERROR',
      message: 'linear alternative is empty; it must list every period and event',
    });
  }
