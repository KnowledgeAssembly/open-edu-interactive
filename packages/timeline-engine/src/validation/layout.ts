...
import { contained } from '../layout/lanes.js';
...
export function validateLayout(scene: Scene, ctx: LayoutContext): ValidationResult {
  const canvas = { x: -200, y: -200, width: ctx.width + 400, height: ctx.height * 3 };
  ...
  for (const node of scene.nodes) {
    ...
    if (node.interactive) {
      const reachable = node.bounds.width >= ctx.minTouchTarget || node.bounds.height >= ctx.minTouchTarget;
      if (!reachable) {
        issues.push({
          level: 'L3',
